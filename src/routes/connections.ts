...
): Promise<{ result: string; latency_ms: number | null; host_key_fingerprint: string | null; message: string }> {
  return new Promise((resolve) => {
    const start = Date.now();
    let hostKeyFP: string | null = null;
    let settled = false;

    const settle = (val: { result: string; latency_ms: number | null; host_key_fingerprint: string | null; message: string }) => {
      if (settled) return;
      settled = true;
      try { conn.end(); } catch {}
      resolve(val);
